...
def build_toc(transcript: Transcript, source: Optional[SourceRef] = None,
              *, segments: int = 8) -> list[Idea]:
    """Chunk the episode into ~equal time segments and label each with its most
    salient line — a cheap deterministic TOC for navigation."""
    if not len(transcript):
        return []
...
def ideas_markdown(ideas: list[Idea], source: Optional[SourceRef] = None) -> str:
    ...
    for idea in ideas:
        ts = format_timestamp(idea.start)
        stamp = f"[{ts}]({idea.deeplink})" if idea.deeplink else f"`{ts}`"
        lines.append(f"- **{stamp}** — {idea.text}")
    lines.append("")
    return "\n".join(lines)
...
def briefing_scaffold(source: SourceRef, transcript: Transcript,
    ...
    for item in toc:
        ...
        stamp = f"[{ts}]({item.deeplink})" if item.deeplink else f"`{ts}`"
